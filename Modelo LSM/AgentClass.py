...
class Agente:
    ...
    __init__:
            ...
            historico: diccionario anidado donde la llave es el numero de iteracion y el valor es
                    un diccionario con la siguiente estructura
                    {sector :{'fenomeno x':{'intensidad': ,'sobre_umbral': ,'señal': }}}
    ...
    def __init__(self,monitoreo):
        ...
        self.historico_forecast = { y:{'sector '+str(x):{z:0 for z in range(p.fenomenos_por_sector) } for x in monitoreo} for y in range(p.numero_iteraciones) }#Arreglar esta parte para tener consistencia
    ...
    def forecast(self,iteracion,sector,fenomeno):
        ...
        if iteracion - p.memoria_agentes < 0:
            ...
            data_historico = [self.historico[i][str(sector)][fenomeno]['intensidad'] for i in range(cuenta_atras,iteracion)]
            # pp.pprint(data_historico)
            data_tiempo = [[i] for i in range(cuenta_atras,iteracion)]
            regr = linear_model.LinearRegression()
            regr.fit(data_tiempo, data_historico)
            predictions = regr.predict([[iteracion]])
            for predi in predictions:
                self.historico_forecast[iteracion][sector][fenomeno] = predi
                # iteracion += 1
            return predictions
